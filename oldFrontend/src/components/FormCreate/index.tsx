...
import { months } from '../../utils/arrays/Login';
import { getDays, getYears } from '../../utils/functions/Login';
...
const Formulario: React.FC<Props> = ({ setShowCreate }) => {
   const { emailRegex, telefoneRegex } = useRegex();
   ...
   const UserHasError = useMemo(
      () => CreateErrors.userAlreadyExists || CreateErrors.userPasswordVeryShort,
      [CreateErrors]
   );
   ...
   async function createUser() {
      ...
      try {
         ...
         switch (err.response.data.error) {
            case 'Usuário já existente com esse email':
               setCreateErrors({
                  ...CreateErrors,
   ...
   function validInput() {
      ...
      let newErrorObject = { ...CreateErrors } as any;

      Object.entries(CreateInput.current).map(item => {
         if (typeof item[1] === 'object') {
            let condicao = item[1].year != dayjs().year();

            newErrorObject[item[0]] = !condicao;

            return;
         }

         if (item[0] === 'password' && item[1].length < 6 && item[1].length >= 1) {
            newErrorObject['userPasswordVeryShort'] = true;
         }
         ...
         switch (item[0]) {
            case 'email':
               if (
                  ...
                  (!isNaN(Number(item[1])) && telefoneRegex.test(item[1]))
               ) {
            ...
            case 'confirmEmail':
               ...
               if (email.length) {
                  if (emailRegex.test(item[1]) || telefoneRegex.test(item[1])) {
                     if (email === item[1]) {
                        newErrorObject[item[0]] && (newErrorObject[item[0]] = false);
                     } else {
                        newErrorObject[item[0]] = true;
                     }
                  } else {
            ...
            case 'gender':
               ...
               GenderAlternatives.personalized && (newErrorObject[item[0]] = false);

               break;

            case 'pronome':
               if (GenderAlternatives.personalized) {
                  if (item[1].length) {
                     newErrorObject[item[0]] = false;
                  } else {
                     newErrorObject[item[0]] = true;
      ...
      setCreateErrors(newErrorObject);
   ...
   function inputsBlurCheckError(name: string) {
   ...
   function confirmEmail(email: string, confirmEmail: string) {
      Object.entries(CreateInput.current).map(item => {
         ...
         const condTelefone = telefoneRegex.test(email) && telefoneRegex.test(confirmEmail);

         if (
            (item[0] === 'confirmEmail' && condEmail) ||
            (item[0] === 'confirmEmail' && condTelefone)
         ) {
                  ...
                  <Email
                     ...
                     onChange={e => {
                        ...
                        if (
                           ...
                           !telefoneRegex.test(e.target.value)
                        ) {
